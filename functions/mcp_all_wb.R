...
mcp_all<-function(vv,title,pop,swd) {
  ...
  migdist <- do.call(rbind, lapply(1:length(uaid), function(j){ #
    ...
    sumyr<-max(year(tmp$date[months(tmp$date)=="January"]))
    winyr<-max(year(tmp$date[months(tmp$date)=="September"]))
    tmp<-tmp[paste(year(tmp$date),months(tmp$date),sep="")==paste((sumyr-1),"December",sep="") |
              paste(year(tmp$date),months(tmp$date),sep="")==paste(sumyr,"January",sep="") |
    ...
    win<-tmp[months(as.POSIXct(tmp$date,tz="MST"))=="August"|months(as.POSIXct(tmp$date,tz="MST"))=="September",]
    sum<-tmp[months(as.POSIXct(tmp$date,tz="MST"))=="December"|months(as.POSIXct(tmp$date,tz="MST"))=="January",]
